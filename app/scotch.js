...
const extractAuthorProfile = $ => {
  ...
  // console.log('extract author profile' , $);
    const mainSite = $('#sitemain');
    // console.log('loggin main site', mainSite);
    // const metaScotch = $("meta**[**property='og:url']");
    // console.log('loggin meta scotch', metaScotch);
    // const scotchHero = mainSite.find('section.hero--scotch');
    const superGrid = null
    const authorTitle = null
    const profileRole = null
    const profileAvatar = null
    const profileStats = null
    const authorLinks = null
    const authorPosts = null

    console.log('about to start logging');
    const scotchHero = mainSite.each((i, r) => {
      authorLinks = scotchHero.find(".author-links");
      console.log('logging author title ', authorLinks);
      authorTitle = scotchHero.find(".profilename h1.title");
      profileAvatar = scotchHero.find("img.profileavatar");
      profileStats = scotchHero.find(".profilestats .profilestat");
      
    })
    // console.log('logging author title ', authorTitle);
    // const superGrid = mainSite.find('section.super-grid');
    superGrid = mainSite.find('section.super-grid');
    // const authorTitle = scotchHero.find(".profilename h1.title");
    // console.log('logging author title', authorTitle);

    // const profileRole = authorTitle.find(".tag");
    profileRole = authorTitle.find(".tag");
    ...
    authorPosts = superGrid.find(".super-griditem **[**data-type='post']");
